...
function main() {
  ...
  //retrieve the infos only once because we use the same program
  ...
  function animate(){
    ...
    //only the last cube is moving, only rtoating, inf fact no translation(0,0,0) or scale (1.0)
  ...
  function drawScene() {
    ...
    for(i = 0; i < 4; i++){
      ...
      //only the last one changes the normals
      if (i < 3) gl.uniformMatrix4fv(normalMatrixPositionHandle, gl.FALSE, utils.transposeMatrix(cubeWorldMatrix[i]));
      //in this case we could have used directly the cubeWorldMatrix[3] because there is no scaling!!! so if (i<4) ... without else is ok too
          //but to be sure we always do the inverse transpose!
