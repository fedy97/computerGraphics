...
var Node = function() {
  this.children = [];
  //from pos of the node to the parent pos obj
  this.localMatrix = utils.identityMatrix();
  //put the obj in world. so chain, from scene graph to the node.
  this.worldMatrix = utils.identityMatrix();
};

/**
 * add method to the Node class
 * @param parent to set for @this node
 */
...
/**
 * recursive function to compute the new world matrixes for all children of the node
 * @param matrix
 */
...
//Define the scene Graph
  ...
  //translate by 100 with respect to its parent, the local matrix is with respect to the parent
  earthOrbitNode.localMatrix = utils.MakeTranslateMatrix(100, 0, 0);

  var moonOrbitNode = new Node();
  //here with respect to the earth orbit node
  ...
  /**
   * objects to be rendered. so that we can iterate on them
   * @type {Node[]}
   */
  var objects = [
    sunNode,
  ...
  function drawScene(time) {
    ...
    // update the local matrices for each object. before drawing them.
    ...
    // Update all world matrices in the scene graph, for all its children.
    sunOrbitNode.updateWorldMatrix();

    // Compute all the matrices for rendering
    objects.forEach(function(object) {
      //here all the 3 objects uses the same program, so same shaders for all the 3.
